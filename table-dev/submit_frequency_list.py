...
def main():
    ...
    c.execute("CREATE TABLE IF NOT EXISTS dictionary (text TEXT PRIMARY KEY, braille TEXT, frequency INTEGER)")
    c.execute("UPDATE dictionary SET frequency = 0")
    for row in reader:
        c.execute("INSERT OR IGNORE INTO dictionary VALUES (:text,Null,:frequency)", row)
        if c.lastrowid == 0:
